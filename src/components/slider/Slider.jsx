...
function Slider() {
  const slideIndex  = useSelector(state => state.sliderSlice.value)
      ...
      <div>
        {sliderData.map((item, index) => (
          <div key={item.id} className={parseInt(item.id) === slideIndex ? 'opacity-100 duration-700 ease-in-out scale-100' : 'opacity-0 duration-700 ease-in-out scale-95'}  >
            ...
            {parseInt(item.id) === slideIndex && <p className='text-white text-4xl font-inter font-bold tracking-normal leading-none'>{item.text}</p>} 
            </div>
          </div>
        ) )}
      </div>
      <div className='flex absolute bottom-12 left-[45%]'>
          {sliderData.map((dot, index) => (
              <div className='mr-4' key={index}>
                <div onClick={() => dispatch(dotSlide(index))} className={index ===  slideIndex ? 'bg-green-300 rounded-full p-4 cursor-pointer' : 'bg-white rounded-full p-4 cursor-pointer' }></div>
              </div>
          ))}
      </div>
            <div>
